...
const Terminal: React.FC<TerminalProps> = ({ currentUser, onNavigate, config, onAddToHistory }) => {
    ...
    const loadHistory = async () => {
        ...
        dbHistory.forEach(item => {
            restoredMessages.push({
                ...
                timestamp: parseInt(item.id) || Date.now()
            });
            restoredMessages.push({
                role: 'model',
  ...
  const startNewChat = () => {
      ...
      setMessages([{
        role: 'model',
        text: `Session Reset. Memory wiped. Connection re-established with ${aiName}.`,
        timestamp: Date.now()
      }]);
      setInput('');
    ...
    try {
      if (input.toLowerCase().includes('dev') || input.toLowerCase().includes('siapa pencipta') || input.toLowerCase().includes('created you')) {
         ...
         setTimeout(() => {
            ...
            saveChatLog(userName, aiName, userMsg.text, devInfo, userMsg.image);
            setIsThinking(false);
         }, 800);
         return;
      }
      ...
      {viewingScript && (
        <div className="absolute inset-0 z-50 bg-black/95 flex flex-col animate-fade-in p-4 md:p-8">
            <div className="flex justify-between items-center mb-4 border-b border-central-secondary pb-4">
                ...
                <button onClick={() => setViewingScript(null)} className="text-red-500 hover:text-white font-bold text-xl px-4">[CLOSE]</button>
            </div>
            <div className="flex-1 bg-[#0d0d0d] border border-gray-700 p-4 overflow-auto custom-scroll relative group">
                <button onClick={() => handleCopyCode(viewingScript.code)} className="absolute top-4 right-4 bg-central-secondary text-white text-xs px-3 py-2 font-pixel opacity-50 group-hover:opacity-100 transition-opacity hover:bg-red-600 border border-black z-10">COPY RAW</button>
                <pre className="font-mono text-sm text-green-400 whitespace-pre-wrap"><code>{viewingScript.code}</code></pre>
            </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-end mb-4 bg-black/80 p-4 border border-central-accent pixel-border shadow-[0_0_15px_rgba(220,38,38,0.2)]">
        <div className="flex flex-col">
            ...
            <div className="font-mono text-[10px] md:text-xs text-gray-500 flex items-center gap-2">
                ...
                <span>ONLINE | USER: {currentUser?.username || 'GUEST'}</span>
            </div>
        </div>
        <div className="flex gap-2">
             <button onClick={startNewChat} className="px-4 py-2 bg-central-secondary text-white text-[10px] font-pixel border border-black hover:bg-red-600 transition-colors shadow-lg">
                + NEW CHAT
            </button>
        </div>
      </div>
                    ...
                    if (part.type === 'code') {
                      ...
                      return (
                        <div key={pIdx} className="my-3 max-w-sm">
                           <div onClick={() => setViewingScript({title: projectTitle, lang: part.language || 'text', code: part.content})} className="group cursor-pointer bg-gray-900 border-l-4 border-central-accent p-4 hover:bg-gray-800 transition-all shadow-[4px_4px_0_rgba(0,0,0,0.5)] active:translate-y-1 active:shadow-none">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 bg-black border border-gray-700 flex items-center justify-center text-2xl">📄</div>
